...
pub struct OrphanImageScanResult {
    pub items: Vec<OrphanImageFileRecord>,
...
pub struct OrphanImageCleanupResult {
    ...
    pub total_bytes: u64,
    pub moved_to_trash_count: usize,
    pub failed: Vec<OrphanImageSkippedFile>,
}
...
fn normalize_tag_color(color: &str) -> String {
    ...
    if normalized.len() == 7
        ...
        && TAG_COLOR_PALETTE.contains(&normalized.as_str())
    {
        normalized
    } else {
...
fn create_tag_record(connection: &Connection, normalized_name: &str) -> Result<TagRecord, String> {
    let color = normalize_tag_color(DEFAULT_TAG_COLOR);
    connection
        .execute(
            "
...
fn validate_note_tag_occurrences(
    ...
    let mut distinct_tag_ids = BTreeSet::new();

    for occurrence in occurrences {
        if occurrence.tag_id <= 0 || !tag_exists(connection, occurrence.tag_id)? {
            return Err("目标标签不存在。".to_string());
        }

        if occurrence.block_id.trim().is_empty()
            || occurrence.node_type.trim().is_empty()
            || occurrence.snippet_text.trim().is_empty()
            || occurrence.start_offset < 0
            || occurrence.end_offset < 0
            || occurrence.start_offset >= occurrence.end_offset
            || occurrence.sort_order < 0
        {
...
fn clear_review_schedule_for_note(transaction: &Connection, note_id: i64) -> Result<(), String> {
    transaction
...
fn write_note_review_schedule(
    ...
    fetch_note_review_schedule(transaction, note_id).map(|schedule| {
        schedule.unwrap_or(NoteReviewScheduleRecord {
            note_id,
            dates: dates.to_vec(),
            updated_at: None,
...
fn reset_note_review_schedule_to_default(
    ...
    let schedule =
        write_note_review_schedule(transaction, note_id, default_plan.id, base_date, &dates)?;
...
fn activate_note_review_schedule_tx_internal(
    ...
    let schedule =
        write_note_review_schedule(&transaction, note_id, default_plan.id, &today, &dates)?;
    transaction
        .commit()
        ...
        if !orphan_note_ids.is_empty() {
            ...
            transaction
                .execute(
                    ...
                    params![
                        notebook_id,
                        recovery_folder_name,
                        ordered_folder_ids.len() as i64
                    ],
                )
...
fn save_note_content_with_tags_tx_internal(
    connection: &mut Connection,
    note_id: i64,
    content: &str,
    occurrences: &[NoteTagOccurrenceInput],
    ...
    let transaction = connection
        .transaction()
        .map_err(|error| to_command_error("开启保存正文事务", error))?;
    let distinct_tag_ids = validate_note_tag_occurrences(&transaction, occurrences)?;
    let updated = transaction
        .execute(
            "
              UPDATE notes
    ...
    transaction
        .execute(
            "DELETE FROM note_tag_occurrences WHERE note_id = ?1",
            [note_id],
        )
        .map_err(|error| to_command_error("清理旧标注索引", error))?;

    if !occurrences.is_empty() {
        let mut insert_occurrence = transaction
            .prepare(
                "
                  INSERT INTO note_tag_occurrences (
                    note_id,
                    tag_id,
                    block_id,
                    start_offset,
    ...
    transaction
        ...
        .map_err(|error| to_command_error("清理旧标签兼容层", error))?;

    if !distinct_tag_ids.is_empty() {
        let mut insert_note_tag = transaction
            .prepare(
                "
            ...
            .map_err(|error| to_command_error("准备写入标签兼容层", error))?;

        for tag_id in distinct_tag_ids {
            insert_note_tag
                .execute(params![note_id, tag_id])
                .map_err(|error| to_command_error("写入标签兼容层", error))?;
        }
    }
...
fn orphan_resource_kind_for_path(resource_path: &str) -> Result<OrphanResourceKind, String> {
    if resource_path.starts_with("resources/images/") {
        return Ok(OrphanResourceKind::Image);
    }

    if resource_path.starts_with("resources/covers/") {
        return Ok(OrphanResourceKind::Cover);
    }

    Err("资源路径无效。".to_string())
}

fn orphan_resource_extension_for_path(resource_path: &str) -> Option<String> {
...
fn should_skip_orphan_scan_file(entry_path: &Path) -> Option<&'static str> {
    let file_name = entry_path.file_name()?.to_str()?;

    if file_name.ends_with(".creating") {
        return Some("临时创建文件不参与正式资源治理。");
    }

    None
}

fn scan_orphan_directory(
    root: &Path,
    directory_name: &str,
    live_resource_paths: &BTreeSet<String>,
    leased_resource_paths: &BTreeSet<String>,
    items: &mut Vec<OrphanImageFileRecord>,
    skipped_files: &mut Vec<OrphanImageSkippedFile>,
) -> Result<(), String> {
    let directory_path = root.join(directory_name);
    ...
    for entry in WalkDir::new(&directory_path).min_depth(1) {
        let entry = entry.map_err(|error| format!("遍历资源目录失败：{error}"))?;

        if entry.file_type().is_symlink() {
            skipped_files.push(OrphanImageSkippedFile {
                path: entry.path().to_string_lossy().to_string(),
                reason: "符号链接不参与正式资源治理。".to_string(),
            });
            continue;
        }
        ...
        if !entry.file_type().is_file() {
            skipped_files.push(OrphanImageSkippedFile {
                path: entry.path().to_string_lossy().to_string(),
                reason: "仅扫描正式资源文件。".to_string(),
            });
            continue;
        }

        if let Some(reason) = should_skip_orphan_scan_file(entry.path()) {
            skipped_files.push(OrphanImageSkippedFile {
                path: entry.path().to_string_lossy().to_string(),
        ...
        let resource_path = match managed_resource_path_from_absolute_path(root, entry.path()) {
        ...
        if !is_app_managed_image_resource_path(&resource_path) {
            skipped_files.push(OrphanImageSkippedFile {
                path: entry.path().to_string_lossy().to_string(),
                reason: "仅扫描应用托管的正式图片资源。".to_string(),
            });
            continue;
        }
...
fn cleanup_orphan_image_resources_internal(
    ...
    let scan = scan_orphan_image_resources_internal(root, connection, leased_resource_paths)?;

    if dry_run {
        return Ok(OrphanImageCleanupResult {
            dry_run: true,
            items: scan.items,
    ...
    let mut moved_to_trash_count = 0;
    let mut failed = Vec::new();

    for item in &scan.items {
        match move_managed_resource_to_trash(
            root,
            &item.resource_path,
            ManagedResourceTrashSource::OrphanCleanup,
            None,
            ...
            Err(error) => {
                failed.push(OrphanImageSkippedFile {
                    path: item.resource_path.clone(),
                    reason: error,
                });
            }
        }
...
#[tauri::command]
pub fn reorder_notebooks_tx(app: AppHandle, ordered_notebook_ids: Vec<i64>) -> Result<(), String> {
    let mut connection = open_database_connection(&app)?;
...
#[tauri::command]
pub fn save_note_content_with_tags_tx(
    app: AppHandle,
    note_id: i64,
    content: String,
...
pub fn cleanup_unreferenced_managed_resources(
    ...
    let result = cleanup_orphan_image_resources(app, false)?;
    Ok(ManagedResourceCleanupResult {
        deleted_count: 0,
        moved_to_trash_count: result.moved_to_trash_count,
        failed: result
            .failed
            .into_iter()
            .map(|failure| ManagedResourceCleanupFailure {
                resource_path: failure.path,
...
pub fn scan_orphan_image_resources(app: AppHandle) -> Result<OrphanImageScanResult, String> {
    ...
    let leased_resource_paths = snapshot_managed_resource_session_leases(&app);
    scan_orphan_image_resources_internal(&root, &connection, &leased_resource_paths)
}
...
pub fn cleanup_orphan_image_resources(
    ...
    let root = resolve_app_root(&app)?;
    let leased_resource_paths = snapshot_managed_resource_session_leases(&app);
    cleanup_orphan_image_resources_internal(&root, &connection, &leased_resource_paths, dry_run)
}
...
mod tests {
    use super::{
        ...
        cleanup_orphan_image_resources_internal, ManagedResourceCleanupResult,
        ManagedResourceTrashSource, NoteTagOccurrenceInput, OrphanImageCleanupResult,
    ...
    ) -> OrphanImageFileRecord {
        OrphanImageFileRecord {
            resource_path: resource_path.to_string(),
            absolute_path: Some(root.join(resource_path).to_string_lossy().to_string()),
            kind,
            size_bytes: 8,
            extension: Path::new(resource_path)
                .extension()
                .and_then(|value| value.to_str())
                .map(|value| value.to_ascii_lowercase()),
            source: None,
    ...
    fn scan_orphan_images(root: &Path, connection: &Connection) -> OrphanImageScanResult {
        scan_orphan_image_resources_internal(root, connection, &BTreeSet::new())
            .expect("scan orphan image resources")
    }
    ...
    fn scan_orphan_images_with_leases(
        ...
        scan_orphan_image_resources_internal(root, connection, leased_resource_paths)
            .expect("scan orphan image resources with leases")
    }
    ...
    fn cleanup_orphan_images(
        ...
        cleanup_orphan_image_resources_internal(root, connection, &BTreeSet::new(), dry_run)
            .expect("cleanup orphan image resources")
    }
    ...
    fn cleanup_candidate_unreferenced_managed_resources_reports_invalid_paths_and_skips_them() {
        ...
        let result = cleanup_candidates_after_resource_grace(
            ...
            &[
                ...
                "resources/./images/kept.png".to_string(),
            ],
        )
    ...
    fn create_note_path_can_commit_three_times() {
        ...
        let ordered_titles = connection
            .prepare("SELECT title FROM notes WHERE folder_id = 1 ORDER BY sort_order ASC, id ASC")
    ...
    fn reorder_notebooks_path_persists_custom_order() {
        ...
        reorder_notebooks_tx_internal(&mut connection, &[3, 1, 2]).expect("reorder notebooks");
    ...
    fn reorder_folders_path_persists_top_level_order() {
        ...
        reorder_folders_tx_internal(&mut connection, 1, &[3, 1, 2]).expect("reorder folders");

        let ordered_ids = connection
            .prepare("SELECT id FROM folders WHERE notebook_id = 1 ORDER BY sort_order ASC, id ASC")
            .expect("prepare ordered folders")
            .query_map([], |row| row.get::<_, i64>(0))
            .expect("query ordered folders")
            .map(|row| row.expect("map ordered folder"))
    ...
    fn ensure_notebook_tree_constraints_path_flattens_legacy_structure() {
        ...
        connection
            .execute(
                "INSERT INTO notebooks (name, custom_sort_order) VALUES ('测试本', 0)",
                [],
            )
            .expect("insert notebook");
        connection
    ...
    fn ensure_review_feature_ready_resets_existing_review_data_to_default() {
        ...
        let plan_name: String = connection
            .query_row("SELECT name FROM review_plans LIMIT 1", [], |row| {
                row.get(0)
            })
    ...
    fn activate_note_review_schedule_generates_default_dates() {
        ...
        let today = today_local_date_key();
        let schedule = activate_note_review_schedule_tx_internal(&mut connection, 1)
    ...
    fn save_note_review_schedule_rejects_past_duplicate_and_invalid_dates() {
        ...
        let past_date = Local::now()
            ...
            .format("%Y-%m-%d")
            .to_string();
        let past_error = save_note_review_schedule_tx_internal(&mut connection, 1, &[past_date])
        ...
        let invalid_error =
            save_note_review_schedule_tx_internal(&mut connection, 1, &["2026-02-31".to_string()])
    ...
    fn save_note_review_schedule_sorts_dates_and_keeps_activation_date() {
        ...
        connection
            ...
            .expect("insert note");
        let activated = activate_note_review_schedule_tx_internal(&mut connection, 1)
            .expect("activate schedule");
    ...
    fn review_rows_cascade_when_note_folder_and_notebook_are_deleted() {
        ...
        activate_note_review_schedule_tx_internal(&mut connection, 2)
            .expect("activate second note");
    ...
    #[test]
    fn normalize_tag_color_accepts_whitespace_and_lowercase_palette_values() {
        assert_eq!(normalize_tag_color("  #ff3b30  "), "#FF3B30");
        assert_eq!(normalize_tag_color("#5ac8fa"), "#5AC8FA");
    }

    #[test]
    fn normalize_tag_color_falls_back_to_default_for_invalid_values() {
        assert_eq!(normalize_tag_color(""), DEFAULT_TAG_COLOR);
        assert_eq!(normalize_tag_color("#123456"), DEFAULT_TAG_COLOR);
        assert_eq!(normalize_tag_color("not-a-color"), DEFAULT_TAG_COLOR);
    }

    #[test]
    fn add_tag_to_note_path_creates_tags_with_default_red_color() {
        let mut connection = test_connection();
        connection
            .execute("INSERT INTO notebooks (name) VALUES ('测试本')", [])
            .expect("insert notebook");
        ...
        let tags =
            add_tag_to_note_by_name_tx_internal(&mut connection, 1, "默认颜色").expect("add tag");
    ...
    #[test]
    fn save_note_content_with_tags_path_rebuilds_occurrences_and_note_tags() {
        let mut connection = test_connection();
        connection
            .execute("INSERT INTO notebooks (name) VALUES ('测试本')", [])
            .expect("insert notebook");
        ...
        connection
            .execute(
                "INSERT INTO tags (name, color) VALUES ('重点', '#FF3B30'), ('定义', '#007AFF')",
                [],
            )
            .expect("insert tags");
        connection
    ...
    fn save_note_content_keeps_removed_image_when_session_leased_and_after_release() {
        ...
        assert_eq!(previous_paths, vec![image_path.to_string()]);
        assert!(leased_scan.items.is_empty());
        assert_eq!(
            released_scan.items,
            vec![orphan_record(
                temp_dir.path(),
                image_path,
                OrphanResourceKind::Image,
            )],
        );
        assert!(temp_dir.path().join(image_path).is_file());
    }
    ...
    #[test]
    fn cleanup_orphan_image_resources_real_run_moves_only_current_orphans_to_trash() {
        let temp_dir = tempdir().expect("create temp dir");
        ensure_test_resource_dirs(temp_dir.path());
    ...
    #[test]
    fn cleanup_orphan_image_resources_real_run_moves_cover_orphan_to_trash() {
        let temp_dir = tempdir().expect("create temp dir");
        ensure_test_resource_dirs(temp_dir.path());
        let orphan_cover_path = "resources/covers/c2c2c2c2-c2c2-42c2-82c2-c2c2c2c2c2c2.png";
        write_test_resource(temp_dir.path(), orphan_cover_path);
        let connection = test_connection();
        ...
        assert_eq!(
            ...
            vec![orphan_record(
                temp_dir.path(),
                orphan_cover_path,
                OrphanResourceKind::Cover,
            )],
        );
        assert_resource_moved_to_trash(temp_dir.path(), orphan_cover_path);
    }
    ...
    #[test]
    fn cleanup_orphan_image_resources_returns_error_when_scan_fails() {
        let temp_dir = tempdir().expect("create temp dir");
        ensure_test_resource_dirs(temp_dir.path());
        let image_path = "resources/images/f3f3f3f3-f3f3-43f3-83f3-f3f3f3f3f3f3.png";
        write_test_resource(temp_dir.path(), image_path);

        let connection = Connection::open_in_memory().expect("open empty sqlite");
        let error = cleanup_orphan_image_resources_internal(
            temp_dir.path(),
            &connection,
            &BTreeSet::new(),
            false,
        )
        .expect_err("scan failure should abort cleanup");

        assert!(
            error.contains("notebooks") || error.contains("读取存活封面引用"),
    ...
    fn save_note_content_with_tags_path_rejects_zero_length_occurrence() {
        ...
        let error = save_note_content_with_tags_tx_internal(
            ...
            &[NoteTagOccurrenceInput {
                tag_id: 1,
                block_id: "blk_zero_len_001".to_string(),
                start_offset: 2,
                end_offset: 2,
                node_type: "paragraph".to_string(),
                snippet_text: "重点".to_string(),
        ...
        assert_eq!(stored_content, "<p>旧正文</p>");
    }

    #[test]
    fn save_note_content_with_tags_path_rejects_empty_snippet() {
        let mut connection = test_connection();
        connection
            .execute("INSERT INTO notebooks (name) VALUES ('测试本')", [])
            .expect("insert notebook");
        ...
        let error = save_note_content_with_tags_tx_internal(
            ...
            &[NoteTagOccurrenceInput {
                tag_id: 1,
                block_id: "blk_empty_snippet_001".to_string(),
                start_offset: 0,
                end_offset: 2,
                node_type: "paragraph".to_string(),
        ...
        .expect_err("empty snippet should fail");

        assert_eq!(error, "标注数据无效，请重新应用标签后再保存。");
    }

    #[test]
    fn save_note_content_with_tags_path_rejects_missing_tag_reference() {
        let mut connection = test_connection();
        connection
            .execute("INSERT INTO notebooks (name) VALUES ('测试本')", [])
            .expect("insert notebook");
    ...
    fn update_notebook_cover_image_path_rejects_invalid_cover_path() {
        ...
        let error =
            update_notebook_cover_image_tx_internal(&mut connection, 1, "/Users/test/cover-a.png")
