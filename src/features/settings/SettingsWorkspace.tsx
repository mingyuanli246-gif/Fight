...
export function SettingsWorkspace({
  ...
  const isAnyStorageOperationBusy = operationState !== "idle";
  ...
  async function handleRestoreBackup() {
    if (restoreDialog?.status !== "confirm" || isAnyStorageOperationBusy) {
      return;
    }
        ...
        <section className={styles.panel}>
          ...
          <section className={styles.resourceCard}>
            <div className={styles.resourceSection}>
              <h4 className={styles.typographyTitle}>存储与删除</h4>
              <p className={styles.typographyDescription}>
                删除的笔记本、文件夹和文件会进入回收站，恢复时其中的图片和封面会一并恢复。永久删除后，相关图片资源会由系统自动清理，不需要手动扫描。
              </p>
            </div>
          </section>
        </section>
